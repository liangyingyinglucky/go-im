...
func (service *ContactService) AddFriend(
	...
	DbEngin.Where("ownerid = ?",userid).
		And("dstid = ?",dstid).
		And("categroy = ?",model.CONCAT_CATE_USER).
		Get(&tmp)
	if tmp.Id>0{
	...
	_,e2 := session.InsertOne(model.Contact{
		Ownerid:userid,
		Dstobj:dstid,
		Categroy:model.CONCAT_CATE_USER,
		Createat:time.Now(),
	})
	//对方的
...
func (service *ContactService) SearchComunity(userId int64) ([]model.Community){
	...
	//跟自己有关系的群
	DbEngin.Where("ownerid = ? and categroy = ?",userId,model.CONCAT_CATE_COMUNITY).Find(&conconts)
...
func (service *ContactService) JoinCommunity(userId,comId int64) error{
	cot := model.Contact{
		Ownerid:userId,
		Dstobj:comId,
		Categroy:model.CONCAT_CATE_COMUNITY,
	}
	DbEngin.Get(&cot)
	if(cot.Id==0){
...
func (service *ContactService) SearchFriend(userId int64) ([]model.User){
	conconts := make([]model.Contact,0)
	objIds :=make([]int64,0)
	DbEngin.Where("ownerid = ? and categroy = ?",userId,model.CONCAT_CATE_USER).Find(&conconts)
	for _,v := range conconts{
